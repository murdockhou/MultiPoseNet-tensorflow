...
def person_detect_train():
    ...
    with graph.as_default():
        ...
        #--------------------------------net-------------------------------------#
        backbone = BackBone(inputs=img_batch, img_size=FLAGS.img_size, batch_size=FLAGS.batch_size, is_training=FLAGS.is_training)
        ...
        loss, decoded_loc_pred = get_loss(img_size=FLAGS.img_size, batch_size=FLAGS.batch_size,
                                          gt_boxes=gt_boxs, loc_pred=loc_pred,
        ...
        restore_share  = tf.train.Saver(var_list=(res50_var_list+fpn_var_list))
        ...
        #-------------------------------tf summary--------------------------------#
        gt_img_batch_with_box    = get_gt_boxs_with_img(imgs=img_batch, gt_boxs=gt_boxs, gt_labels=gt_labels,
                                                        batch_size=FLAGS.batch_size, img_size=FLAGS.img_size)
        pred_img_batch_with_box  = get_pred_boxs_with_img(imgs=img_batch, decoded_boxs=decoded_loc_pred, cls_pred=cls_pred,
                                                          batch_size=FLAGS.batch_size, img_size=FLAGS.img_size)
        ...
        tf.summary.image('Pre_bbox', pred_img_box_placeholder, max_outputs=1)
                ...
                while not coord.should_stop() and step < 300000:
                    ...
                    _, lr= sess.run(
                        [gt_img_batch_with_box, pred_img_batch_with_box,
                         ...
                         apply_gradient_op, learning_rate
                         ]
                    )
                    # cur_time = time.time()
